...
select round(avg(salary), 0) as `평균연봉` from emp;

-- COMMAND ----------

-- DBTITLE 1,max(): 최대값
select max(salary) as `최대연봉` from emp;

-- COMMAND ----------

-- DBTITLE 1,min(): 최소값
select min(salary) as `최소연봉` from emp;

-- COMMAND ----------

-- DBTITLE 1,sum(): 합계
select sum(salary) as `연봉합계` from emp;
...
from emp 
group by deptno -- 부서별
having min(sal)>(select min(sal) from emp where deptno='30'); --부서별 최소 급여 > (30번 부서 최소 급여)
...
select * from dev.department

-- COMMAND ----------

-- DBTITLE 1,조건문(deptno가 같은 것)에 기반하여 두 테이블 모두에 충족하는 값(deptno 1,2,3)만 출력
SELECT id, name, employee.deptno, dedeptname
FROM dev.employee
INNER JOIN dev.department ON dev.employee.deptno = dev.department.deptno;
...
FROM dev.employee
JOIN dev.department ON dev.employee.deptno = dev.department.deptno;

-- COMMAND ----------

-- DBTITLE 1,예제1: 같은 도시에 거주하는 sales_id와 customer_id 찾기(출력은 salesman 테이블의 name, customer 테이블의 cust_name, city)
use megazone;
SELECT name, customer.cust_name, customer.city 
FROM salesman 
...
SELECT id, name, department.deptno, deptname
FROM dev.employee
FULL JOIN dev.department ON employee.deptno = department.deptno;
